...
SPARCS_PLP_RUST_CARGO_SITE = $(call qstrip,$(BR2_PACKAGE_SPARCS_PLP_RUST_CARGO_REPO_URL))
SPARCS_PLP_RUST_CARGO_SITE_METHOD = git
VERSION = $(call qstrip,$(BR2_PACKAGE_SPARCS_PLP_RUST_CARGO_REPO_VERSION))

# If the version specified is a branch name, we need to go fetch the SHA1 for the branch's HEAD
ifeq ($(shell git ls-remote --heads $(SPARCS_PLP_RUST_CARGO_SITE) $(VERSION) | wc -l), 1)
    SPARCS_PLP_RUST_CARGO_VERSION := $(shell git ls-remote $(SPARCS_PLP_RUST_CARGO_SITE) $(VERSION) | cut -c1-8)
else
    SPARC_PLP_RUST_CARGO_VERSION = $(VERSION)
endif

define SPARCS_PLP_RUST_CARGO_INSTALL_TARGET_CMDS
